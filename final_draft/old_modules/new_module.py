...
def download_stock_price_data(tickers, start_date, end_date):
    """
    Download close prices and compute multiplicative daily price changes.
    ...
    start_date : str
        First date requested from Yahoo Finance, formatted as ``YYYY-MM-DD``.
    end_date : str
        Last date requested from Yahoo Finance, formatted as ``YYYY-MM-DD``.
...
def price_momentum(price_series, lookback_window):
    ...
    Momentum is defined as price_t / price_{t-lookback_window} - 1.  Positive
    values therefore mean that the asset price increased over the lookback
    period.
...
def annualized_volatility(returns, periods_per_year=250):
    ...
    The variance is calculated explicitly with NumPy arithmetic instead of a
    hidden rolling or summary method.  The result is scaled by the square root
    of the number of periods per year.
    """
...
def sharpe_ratio(returns, periods_per_year=250):
    ...
    The statistic measures average excess return per unit of annualized
    volatility.  The project uses a zero risk-free rate, so the numerator is
...
def maximum_drawdown(portfolio_values):
    ...
    The returned value is usually negative; for example, -0.25 means that the
    portfolio lost 25 percent from a previous peak.
    """
    values = np.asarray(portfolio_values, dtype=float)
...
def performance_statistics_from_values(portfolio_values, periods_per_year=250):
    ...
    drawdown in a dictionary that can be converted directly into a table in the
    notebooks.
    """
    values = np.asarray(portfolio_values, dtype=float)
...
def signal_dataframe(index, raw_signal, **extra_columns):
    ...
    non-positive or non-finite signal to 0.0.  This keeps all strategies in the
    same binary format: invested or not invested.
    ...
    **extra_columns : array-like
        Optional diagnostic series, such as momentum or moving averages.
...
def ita_defensive_volatility_signal(
    ...
    """
    Build an aerospace-defense signal with market-stress confirmation.

    Economic idea: the strategy invests in ITA when ITA is trending upward,
    has enough absolute momentum, outperforms SPY, and either market volatility
    is elevated or defensive sectors show strength relative to the market.
    """
